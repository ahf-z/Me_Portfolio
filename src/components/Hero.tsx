
import { useState, useEffect, useRef } from 'react';
import { Button } from '@/components/ui/button';
import { ArrowDown, Github, Linkedin, Mail } from 'lucide-react';
import anime from 'animejs/lib/anime.es.js';

const Hero = () => {
  const [currentRole, setCurrentRole] = useState(0);
  const heroRef = useRef<HTMLDivElement>(null);
  const titleRef = useRef<HTMLHeadingElement>(null);
  const roleRef = useRef<HTMLHeadingElement>(null);
  const descriptionRef = useRef<HTMLParagraphElement>(null);
  const buttonsRef = useRef<HTMLDivElement>(null);
  const socialRef = useRef<HTMLDivElement>(null);
  
  const roles = [
    'Full-Stack Developer',
    'SAP Fiori Specialist',
    'Cloud & AI Enthusiast'
  ];

  useEffect(() => {
    // Initial load animations
    const tl = anime.timeline({
      easing: 'easeOutExpo',
      duration: 800
    });

    tl.add({
      targets: titleRef.current,
      translateY: [50, 0],
      opacity: [0, 1],
      duration: 1000
    })
    .add({
      targets: roleRef.current,
      translateY: [30, 0],
      opacity: [0, 1],
      duration: 800
    }, '-=600')
    .add({
      targets: descriptionRef.current,
      translateY: [30, 0],
      opacity: [0, 1],
      duration: 800
    }, '-=400')
    .add({
      targets: buttonsRef.current,
      translateY: [30, 0],
      opacity: [0, 1],
      duration: 800
    }, '-=400')
    .add({
      targets: socialRef.current?.children,
      scale: [0, 1],
      opacity: [0, 1],
      duration: 600,
      delay: anime.stagger(100)
    }, '-=200');

    // Background elements animation
    anime({
      targets: '.bg-float',
      translateY: [-20, 20],
      duration: 4000,
      direction: 'alternate',
      loop: true,
      easing: 'easeInOutSine',
      delay: anime.stagger(1000)
    });

    const interval = setInterval(() => {
      setCurrentRole((prev) => (prev + 1) % roles.length);
    }, 3000);

    return () => clearInterval(interval);
  }, []);

  useEffect(() => {
    // Role change animation
    if (roleRef.current) {
      anime({
        targets: roleRef.current,
        scale: [1, 1.05, 1],
        duration: 600,
        easing: 'easeOutElastic(1, .8)'
      });
    }
  }, [currentRole]);

  const scrollToContact = () => {
    anime({
      targets: 'html, body',
      scrollTop: document.getElementById('contact')?.offsetTop || 0,
      duration: 1000,
      easing: 'easeInOutCubic'
    });
  };

  const scrollToAbout = () => {
    anime({
      targets: 'html, body',
      scrollTop: document.getElementById('about')?.offsetTop || 0,
      duration: 1000,
      easing: 'easeInOutCubic'
    });
  };

  const handleSocialHover = (e: React.MouseEvent) => {
    anime({
      targets: e.currentTarget,
      scale: 1.2,
      rotate: '5deg',
      duration: 300,
      easing: 'easeOutElastic(1, .5)'
    });
  };

  const handleSocialLeave = (e: React.MouseEvent) => {
    anime({
      targets: e.currentTarget,
      scale: 1,
      rotate: '0deg',
      duration: 300,
      easing: 'easeOutElastic(1, .5)'
    });
  };

  return (
    <section id="home" className="min-h-screen flex items-center justify-center relative overflow-hidden" ref={heroRef}>
      {/* Background Elements */}
      <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-background to-accent/5"></div>
      <div className="absolute top-20 left-10 w-72 h-72 bg-primary/10 rounded-full mix-blend-multiply filter blur-xl opacity-70 bg-float"></div>
      <div className="absolute top-40 right-10 w-72 h-72 bg-accent/10 rounded-full mix-blend-multiply filter blur-xl opacity-70 bg-float"></div>
      <div className="absolute -bottom-8 left-20 w-72 h-72 bg-secondary/10 rounded-full mix-blend-multiply filter blur-xl opacity-70 bg-float"></div>

      <div className="relative z-10 text-center px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        <div>
          <h1 
            ref={titleRef}
            className="text-4xl md:text-6xl lg:text-7xl font-bold text-foreground mb-6 font-poppins opacity-0"
          >
            Hi, I'm{' '}
            <span className="gradient-text">Ahfaz Abdul</span>
          </h1>
          
          <div className="h-16 md:h-20 flex items-center justify-center mb-8">
            <h2 
              ref={roleRef}
              className="text-xl md:text-3xl lg:text-4xl text-muted-foreground font-medium opacity-0"
            >
              {roles[currentRole]}
            </h2>
          </div>

          <p 
            ref={descriptionRef}
            className="text-lg md:text-xl text-muted-foreground mb-12 max-w-3xl mx-auto leading-relaxed opacity-0"
          >
            3+ years of experience building scalable web applications with React, Next.js, SAP Fiori, 
            and cloud technologies. Passionate about creating innovative solutions that make a difference.
          </p>

          <div 
            ref={buttonsRef}
            className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-12 opacity-0"
          >
            <Button
              size="lg"
              onClick={scrollToContact}
              className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-3 text-lg font-medium rounded-full transition-all duration-300 transform hover:scale-105"
            >
              Get In Touch
            </Button>
            
            <Button
              variant="outline"
              size="lg"
              onClick={scrollToAbout}
              className="px-8 py-3 text-lg font-medium rounded-full glass-effect hover:bg-accent/50 transition-all duration-300"
            >
              Learn More
            </Button>
          </div>

          <div ref={socialRef} className="flex justify-center space-x-6">
            <a
              href="https://linkedin.com/in/ahfaz-abdul"
              target="_blank"
              rel="noopener noreferrer"
              className="p-3 rounded-full glass-effect hover:bg-accent/50 transition-all duration-300"
              onMouseEnter={handleSocialHover}
              onMouseLeave={handleSocialLeave}
            >
              <Linkedin className="h-6 w-6" />
            </a>
            <a
              href="https://github.com/ahfaz-abdul"
              target="_blank"
              rel="noopener noreferrer"
              className="p-3 rounded-full glass-effect hover:bg-accent/50 transition-all duration-300"
              onMouseEnter={handleSocialHover}
              onMouseLeave={handleSocialLeave}
            >
              <Github className="h-6 w-6" />
            </a>
            <a
              href="mailto:ahfaz.abdul@email.com"
              className="p-3 rounded-full glass-effect hover:bg-accent/50 transition-all duration-300"
              onMouseEnter={handleSocialHover}
              onMouseLeave={handleSocialLeave}
            >
              <Mail className="h-6 w-6" />
            </a>
          </div>
        </div>

        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2">
          <ArrowDown className="h-8 w-8 text-muted-foreground animate-bounce" />
        </div>
      </div>
    </section>
  );
};

export default Hero;
